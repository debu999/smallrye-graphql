...
public class JsonBCreator {
  ...
  private static final Set<JsonbSerializer> GRAPH_SERIALIZERS = new HashSet<>(
      Arrays.asList(CUSTOM_STRING_SERIALIZER, CUSTOM_INT_SERIALIZER, CUSTOM_FLOAT_SERIALIZER));
  private static final Set<JsonbDeserializer> GRAPH_DESERIALIZERS = new HashSet<>(
      Arrays.asList(CUSTOM_STRING_DESERIALIZER, CUSTOM_INT_DESERIALIZER,
  ...
  private static Jsonb JSONB = JsonbBuilder.create(new JsonbConfig().withFormatting(true)
      ...
      .withSerializers(GRAPH_SERIALIZERS.toArray(new JsonbSerializer[0]))
      .withDeserializers(GRAPH_DESERIALIZERS.toArray(new JsonbDeserializer[0])));
  ...
  /*
   * New Setter to set Custom JSONB config with additional serializers and deserializers
   */
  public static void setJsonB(List<JsonbSerializer> serializers,
      List<JsonbDeserializer> deserializers) {
    GRAPH_SERIALIZERS.addAll(serializers);
    GRAPH_DESERIALIZERS.addAll(deserializers);
    GRAPH_SERIALIZERS.removeIf(Objects::isNull);
    GRAPH_DESERIALIZERS.removeIf(Objects::isNull);
  ...
  private static Jsonb createJsonB(Map<String, String> customFieldNameMapping) {
    ...
    JsonbConfig config = createDefaultConfig().withPropertyNamingStrategy(
        new GraphQLNamingStrategy(customFieldNameMapping));
  ...
  static class CustomScalarSerializers {
    ...
    /**
     * A serializer for CustomScalars based on GraphQL Strings, to inform JsonB how to serialize a
     * CustomStringScalar to a String value.
     */
        ...
        try {
          if (jsonParser.getValue().getValueType() == ValueType.NULL) {
            ...
            return (CustomStringScalar) classloadingService.loadClass(type.getTypeName())
                .getConstructor(String.class).newInstance(jsonParser.getString());
          }
        } catch (Exception e) {
    ...
    /**
     * A serializer for CustomScalars based on a GraphQL Int, to inform JsonB how to serialize a
     * CustomStringScalar to a BigInteger value.
     */
        ...
        try {
          if (jsonParser.getValue().getValueType() == ValueType.NULL) {
            ...
            return (CustomIntScalar) classloadingService.loadClass(type.getTypeName())
                .getConstructor(Integer.class).newInstance(jsonParser.getInt());
          }
        } catch (Exception e) {
          throw new RuntimeException(e);
        }
    ...
    /**
     * A serializer for CustomScalars based on a GraphQL Float, to inform JsonB how to serialize a
     * CustomStringScalar to a BigDecimal value.
     */
        ...
        try {
          if (jsonParser.getValue().getValueType() == ValueType.NULL) {
            ...
            return (CustomFloatScalar) classloadingService.loadClass(type.getTypeName())
                .getConstructor(BigDecimal.class).newInstance(jsonParser.getBigDecimal());
          }
        } catch (Exception e) {
          throw new RuntimeException(e);
        }
